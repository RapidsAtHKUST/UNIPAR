...
void * compact_push_update_intra_push_gpu (void * arg)
{
	...
	for (i=0; i<num_of_partitions; i++)
	{
		...
		push_mssg_offset_compact <<<block_size, THREADS_PER_BLOCK_NODES, sizeof(vid_t)*(total_num_partitions+1)>>> (size, total_num_partitions, pid, jindex_offset[i], jnb_index_offset[i]);
	}
#ifdef MEASURE_TIME_
	CUDA_CHECK_RETURN (cudaDeviceSynchronize());
	...
	for (i=0; i< num_of_partitions; i++)
	{
		...
		int num_of_blocks = (size + THREADS_PER_BLOCK_NODES - 1) / THREADS_PER_BLOCK_NODES;
		int block_size = num_of_blocks > MAX_NUM_BLOCKS ? MAX_NUM_BLOCKS : num_of_blocks;
		push_mssg_compact <<<block_size, THREADS_PER_BLOCK_NODES, sizeof(vid_t)*(total_num_partitions+1)>>> (size, total_num_partitions, pid, jindex_offset[i], jnb_index_offset[i]);
	}
#ifdef MEASURE_TIME_
	CUDA_CHECK_RETURN (cudaDeviceSynchronize());
	gettimeofday (&end, NULL);
...
//	CUDA_CHECK_RETURN (cudaMemcpyAsync(mst->receive[did], (compact_t *)cm->receive+inter_start, (inter_end-inter_start)*sizeof(compact_t), cudaMemcpyDeviceToHost, streams[did]));
	...
	for (i=0; i<num_of_partitions; i++)
	{
		...
		int num_of_blocks = (num_mssgs + THREADS_PER_BLOCK_NODES - 1) / THREADS_PER_BLOCK_NODES;
		int block_size = num_of_blocks > MAX_NUM_BLOCKS ? MAX_NUM_BLOCKS : num_of_blocks;
		pull_update <<<block_size, THREADS_PER_BLOCK_NODES>>> (num_mssgs, pid, jindex_offset[i], jnb_index_offset[i], cm->receive, 1);
	}
#ifdef MEASURE_TIME_
	CUDA_CHECK_RETURN (cudaDeviceSynchronize());
	gettimeofday (&end, NULL);
...
void * update_inter_pull_gpu (void * arg)
{
	...
	for (i=0; i<num_of_partitions; i++)
	{
		...
		int num_of_blocks = (num_mssgs + THREADS_PER_BLOCK_NODES - 1) / THREADS_PER_BLOCK_NODES;
		int block_size = num_of_blocks > MAX_NUM_BLOCKS ? MAX_NUM_BLOCKS : num_of_blocks;
		pull_update <<<block_size, THREADS_PER_BLOCK_NODES>>> (num_mssgs, pid, jindex_offset[i], jnb_index_offset[i], (char*)cm->receive + sizeof(compact_t) * receive_start, 0);
	}
#ifdef MEASURE_TIME_
	CUDA_CHECK_RETURN (cudaDeviceSynchronize());
	gettimeofday (&end, NULL);
...
//	printf ("+++++++++ DST NOT FOUND IN FIRST ITERATION: %u\n", dst_not_found);
	...
	for (i=0; i<num_of_partitions; i++)
	{
		...
		int num_of_blocks = (size + THREADS_PER_BLOCK_NODES - 1) / THREADS_PER_BLOCK_NODES;
		int block_size = num_of_blocks > MAX_NUM_BLOCKS ? MAX_NUM_BLOCKS : num_of_blocks;
		update_ulens_with_kplus1 <<<block_size, THREADS_PER_BLOCK_NODES, sizeof(vid_t)*(total_num_partitions+1)>>> (size, pid, jindex_offset[i], jnb_index_offset[i], k, total_num_partitions);
	}
...
/*	printf ("WORLD RANK %d::::::: RRRRRRRRRRRECHECK INDEX_OFFSETS::::::::", did);
	...
	for (i=0; i<num_of_partitions; i++)
	{
		...
		int num_of_blocks = (num_mssgs + THREADS_PER_BLOCK_NODES - 1) / THREADS_PER_BLOCK_NODES;
		int block_size = num_of_blocks > MAX_NUM_BLOCKS ? MAX_NUM_BLOCKS : num_of_blocks;
		pull_mssg_contig <<<block_size, THREADS_PER_BLOCK_NODES>>> (num_mssgs, pid, jindex_offset[i], jnb_index_offset[i], 0, 1, k);
	}
#ifdef MEASURE_TIME_
	CUDA_CHECK_RETURN (cudaDeviceSynchronize());
	gettimeofday (&end, NULL);
...
void * gather_contig_inter_pull_gpu (void * arg)
{
	...
	for (i=0; i<num_of_partitions; i++)
	{
		...
		int num_of_blocks = (num_mssgs + THREADS_PER_BLOCK_NODES - 1) / THREADS_PER_BLOCK_NODES;
		int block_size = num_of_blocks > MAX_NUM_BLOCKS ? MAX_NUM_BLOCKS : num_of_blocks;
		pull_mssg_contig <<<block_size, THREADS_PER_BLOCK_NODES>>> (num_mssgs, pid, jindex_offset[i], jnb_index_offset[i], receive_start, 0, k);
	}
#ifdef MEASURE_TIME_
	CUDA_CHECK_RETURN (cudaDeviceSynchronize());
	gettimeofday (&end, NULL);
	...
	for (i=0; i<num_of_partitions; i++)
	{
		...
		int num_of_blocks = (size + THREADS_PER_BLOCK_NODES - 1) / THREADS_PER_BLOCK_NODES;
		int block_size = num_of_blocks > MAX_NUM_BLOCKS ? MAX_NUM_BLOCKS : num_of_blocks;
		complete_contig_with_junction_gpu <<<block_size, THREADS_PER_BLOCK_NODES>>> (size, pid, jindex_offset[i], jnb_index_offset[i], k, cutoff);
	}
...
//	printf ("+++++++++++++++++++WORLD RANK %d: TOTAL NUMBER OF KMERS NOT FOUND ON GPUS: %lu\n", world_rank, total_not_found);
