...
void * mpi_allgatherv_inplace (void * send, void * receive, int total_num_of_partitions, int world_size, int world_rank, int unit_size)
{
	if (world_size == 1)
		return NULL;

	void * tmp_buf = (long long *) malloc (sizeof(long long) * total_num_of_partitions);
	...
	if (unit_size==4)
		...
		MPI_Allgatherv(send, np_node, MPI_LONG, tmp_buf, rcounts, rdispls, MPI_LONG, MPI_COMM_WORLD);
	else
	{
		printf ("use MPI_CHAR in mpi_allgatherv!\n");
		MPI_Allgatherv(send, np_node, MPI_CHAR, tmp_buf, rcounts, rdispls, MPI_CHAR, MPI_COMM_WORLD);
	}

	memcpy(receive, tmp_buf, unit_size * total_num_of_partitions);
	free (tmp_buf);
	return receive;
}

void * mpi_allgatherv (void * send, void * receive, int total_num_of_partitions, int world_size, int world_rank, int unit_size)
{
	if (world_size == 1)
		return NULL;
	...
	if (unit_size==4)
		...
		MPI_Allgatherv(send, np_node, MPI_LONG, receive, rcounts, rdispls, MPI_LONG, MPI_COMM_WORLD);
	else
	{
		printf ("use MPI_CHAR in mpi_allgatherv!\n");
		MPI_Allgatherv(send, np_node, MPI_CHAR, receive, rcounts, rdispls, MPI_CHAR, MPI_COMM_WORLD);
	}
...
//		printf ("\n");
	...
	mpi_allgatherv(mst->partition_list, tmp_partition_list, total_num_of_partitions, world_size, world_rank, sizeof(int));
