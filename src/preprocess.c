...
static ull * spids; // for output shared pids of neighbors of kmers
static ull * spidsr; // for output shared pids of neighbors of reverse complements
static uint * spidlv; // for output shared pids of post and pre of linear vertices
...
//	lkmers = dbm->dls.kmers;
	...
	spids = dbm->djs.spids;
	spidsr = dbm->djs.spidsr;
	spidlv = dbm->dls.spids;
}
...
//	printf ("index offset on CPU %d: \n", did);
//	print_offsets(index_offset, num_of_partitions);
}
...
/*		int pid = query_partition_id_from_idoffsets(mssg->srcid, 32, id_offsets);
		if (mssg->srcid >= id_offsets[pid] && mssg->srcid < id_offsets[pid] + jid_offset[pid]) // a junction id
			receive_junction++;
		else
			receive_linear++;*/
	}
...
//		printf ("num_of_elems = %u, size_prime_index[%d]= %u\n", num_of_elems, i, size_prime_index[i]);
		free (tbs[pid].buf);
	}
	index_offset[i] = offset;
//	printf ("index offset on CPU %d: \n", did);
...
{
	if (edge>=4)
		printf("Error in getting adj id from post: edge must be a number in [0-3]!\n");
...
//					local_offset = local_send_offsets[pindex+1]++;
					...
					if (tmp.srcid >= 0 && tmp.srcid < jid_offset[curr_id]) // junction
						send_junction++;
					else
						send_linear++;
					buf[off] = tmp;
...
#pragma omp parallel for num_threads(cpu_threads)
	for (r = 0; r < size; r++)
	{
		...
		if (jflag==true) // a junction
			local_vs[index].vid = local_jvalid[index];
		else if (lflag) // a linear vertex
			local_vs[index].vid = jid_offset[pid] + local_lvalid[index];
	}
}
...
static void gather_vertex_partitioned (uint size, int pid, voff_t index_offset, int total_num_partitions, int k, int p, voff_t max_jsize, voff_t max_lsize)
{
	vertex_t * local_vs = vertices + index_offset;
	uint count=0;
	uint dead=0;
	uint r;
	memset (spids, 0, sizeof(ull) * max_jsize);
	memset (spidsr, 0, sizeof(ull) * max_jsize);
	memset (spidlv, 0, sizeof(uint) * max_lsize);
#pragma omp parallel for num_threads(cpu_threads) firstprivate(pid, size, local_vs)
	for (r = 0; r < size; r++)
	{
		int i;
		...
		voff_t off = local_vs[index].vid - 1;

		if (local_vs[index].vid <= jid_offset[pid]) // a junction here: do not use is_junction for its <=
		{
			for (i=0; i<EDGE_DIC_SIZE / 2; i++)
			{
				if ((local_vs[index].edge >> (i*8) & 0xff) >= cutoff)
				{
					spids[off] |= (ull)(get_adj_id_from_post (&local_vs[index].kmer, (edge_type) i, k, p, total_num_partitions)) << (i*SPID_BITS);
				}
				adj_nbs[i][off] = local_vs[index].nbs[i];
			}
			for (i=EDGE_DIC_SIZE / 2; i<EDGE_DIC_SIZE; i++)
			{
				if ((local_vs[index].edge >> (i*8) & 0xff) >= cutoff)
				{
					spidsr[off] |= (ull)(get_adj_id_from_pre (&local_vs[index].kmer, (edge_type) (i-EDGE_DIC_SIZE/2), k, p, total_num_partitions)) << ((i-EDGE_DIC_SIZE/2)*SPID_BITS);
				}
				adj_nbs[i][off] = local_vs[index].nbs[i];
			}
			jkmers[off] = local_vs[index].kmer;
		...
		else // a linear vertex
		{
			for (i = 0; i < EDGE_DIC_SIZE / 2; i++)
			{
				if ((local_vs[index].edge >> (i*8) & 0xff) >= cutoff)
				{
					spidlv[off - jid_offset[pid]] |= (get_adj_id_from_post (&local_vs[index].kmer, (edge_type) i, k, p, total_num_partitions));
					posts[off - jid_offset[pid]] = local_vs[index].nbs[i];
					post_edges[off - jid_offset[pid]] = i;
					break;
				}
			...
			for (i = EDGE_DIC_SIZE / 2; i < EDGE_DIC_SIZE; i++)
			{
				if ((local_vs[index].edge >> (i*8) & 0xff) >= cutoff)
				{
					spidlv[off - jid_offset[pid]] |= get_adj_id_from_pre (&local_vs[index].kmer, (edge_type) (i-EDGE_DIC_SIZE/2), k, p, total_num_partitions) << SPID_BITS;
					pres[off - jid_offset[pid]] = local_vs[index].nbs[i];
					pre_edges[off - jid_offset[pid]] = i - EDGE_DIC_SIZE/2;
					break;
				}
...
//		gather_vertex_cpu (size, pid, index_offset[i], total_num_partitions);
		gather_vertex_partitioned (size, pid, index_offset[i], total_num_partitions, k, p, gmax_jsize, gmax_lsize);
		uint jsize = mst->jid_offset[pid];
		uint lsize = mst->id_offsets[pid+1] - mst->id_offsets[pid] - jsize;
//		write_junctions_cpu (dbm, mst, jsize, lsize, pid, total_num_partitions, did);
//		write_linear_vertices_cpu (dbm, mst, jsize, lsize, pid, total_num_partitions, did);
...
//	write_ids_cpu (dbm, mst, num_of_partitions, did);
