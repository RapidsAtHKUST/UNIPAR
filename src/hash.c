...
uint atomic_and_int (uint * address, uint value)
{
	uint ret = atomic_fetch_and (address, value);
	return ret;
}

uint atomic_or_int (uint * address, uint value)
{
	uint ret = atomic_fetch_or (address, value);
	return ret;
}
