...
void init_host_filter2 (dbmeta_t * dbm, master_t * mst, vid_t max_subsize)
{
	...
	for (i = num_of_devices; i < num_of_cpus+num_of_devices; i++)
	{
		...
		dbm[i].id_offsets = (goffset_t *) malloc (sizeof(goffset_t) * (total_num_partitions + 1));
		CHECK_PTR_RETURN(dbm[i].id_offsets, "malloc global id offsets error!\n");
		dbm[i].jid_offset = (goffset_t *) malloc (sizeof(goffset_t) * total_num_partitions);
...
void set_id_offsets_cpu (dbmeta_t * dbm, master_t * mst)
{
	...
	for (i=num_of_devices; i<num_of_devices+num_of_cpus; i++)
	{
		memcpy (dbm[i].id_offsets, mst->id_offsets, sizeof(goffset_t) * (total_num_partitions + 1));
		memcpy (dbm[i].jid_offset, mst->jid_offset, sizeof(goffset_t) * total_num_partitions);
	}
}
...
void init_host_gather (dbmeta_t * dbm, master_t * mst, vid_t max_subsize, vid_t max_jsize, vid_t max_lsize)
{
	...
	for (i = num_of_devices; i < num_of_cpus+num_of_devices; i++)
	{
		...
		dbm[i].djs.spids = (ull*) malloc (sizeof(ull) * max_jsize);
		dbm[i].djs.spidsr = (ull *) malloc (sizeof(ull) * max_jsize);
		CHECK_PTR_RETURN (dbm[i].djs.spids, "malloc spids for junctions error!\n");
		CHECK_PTR_RETURN (dbm[i].djs.spidsr, "malloc spidsr for junctions error!\n");
		...
		dbm[i].dls.spids = (uint *) malloc (sizeof(uint) * max_lsize);
		CHECK_PTR_RETURN (dbm[i].dls.spids, "malloc spids for linear vertices error!\n");
...
void finalize_host_gather2 (dbmeta_t * dbm, master_t * mst)
{
	...
	for (i = num_of_devices; i < num_of_cpus+num_of_devices; i++)
	{
		...
		free(dbm[i].djs.spids);
		free(dbm[i].djs.spidsr);
//		free(dbm[i].dls.kmers);
		free(dbm[i].dls.posts);
		free(dbm[i].dls.pres);
		free(dbm[i].dls.spids);
...
//		int num_of_partitions = mst->num_partitions[i + 1] - mst->num_partitions[i];
		...
		dm[i].edge.spid_nb = (uint *) malloc (sizeof(uint) * gsize);
		CHECK_PTR_RETURN (dm[i].edge.spid_nb, "malloc shared partition id for pre and post error!\n");
		dm[i].edge.spid_js = (uint *) malloc (sizeof(uint) * gsize);
		CHECK_PTR_RETURN (dm[i].edge.spid_js, "malloc shared partition id for fjid and bjid error!\n");
		memset (dm[i].edge.spid_nb, 0, sizeof(uint) * gsize);
		memset (dm[i].edge.spid_js, 0, sizeof(uint) * gsize);
		...
		dm[i].id_offsets = (goffset_t *) malloc (sizeof(goffset_t) * (total_num_partitions + 1));
		CHECK_PTR_RETURN (dm[i].id_offsets, "malloc id_offsets for cpu %d error!\n", i);
		dm[i].jid_offset = (goffset_t *) malloc (sizeof(goffset_t) * total_num_partitions);
		...
		memcpy (dm[i].id_offsets, mst->id_offsets, sizeof(goffset_t) * (total_num_partitions + 1));
		memcpy (dm[i].jid_offset, mst->jid_offset, sizeof(goffset_t) * total_num_partitions);
...
void finalize_host_graph_compute (meta_t * dm, master_t * mst)
{
	...
	for (i = num_of_devices; i < num_of_cpus+num_of_devices; i++)
	{
		...
		free (dm[i].edge.fjid);
		free (dm[i].edge.bjid);
		free (dm[i].edge.spid_nb);
		free (dm[i].edge.spid_js);
...
void realloc_host_junctions (meta_t * dm, master_t * mst)
{
	...
	for (i=num_of_devices; i<num_of_devices+num_of_cpus; i++)
	{
		...
		dm[i].junct.spids = (uint *) malloc (sizeof(uint) * spids_size);
		...
		CHECK_PTR_RETURN (dm->junct.spids, "realloc junct shared pids error!\n");
		...
		memset (dm[i].junct.spids, 0, sizeof(uint) * spids_size);
