...
//		CUDA_CHECK_RETURN (cudaMalloc (&dbm[i].sorted_kmers, sizeof(ull) * gsize));
		...
		CUDA_CHECK_RETURN (cudaMalloc (&dbm[i].id_offsets, sizeof(goffset_t) * (total_num_partitions + 1)));
		CUDA_CHECK_RETURN (cudaMalloc (&dbm[i].jid_offset, sizeof(goffset_t) * total_num_partitions));
//		CUDA_CHECK_RETURN (cudaMalloc (&dbm[i].nds.spids, sizeof(ull) * gsize));
//		CUDA_CHECK_RETURN (cudaMalloc (&dbm[i].nds.spidsr, sizeof(ull) * gsize));
//		CUDA_CHECK_RETURN (cudaMemset (dbm[i].nds.spids, 0, sizeof(ull) * gsize));
//		CUDA_CHECK_RETURN (cudaMemset (dbm[i].nds.spidsr, 0, sizeof(ull) * gsize));
...
static void set_id_offsets_gpu (dbmeta_t * dbm, master_t * mst)
{
	...
	for (i=0; i<num_of_devices; i++)
	{
		...
		cudaSetDevice(i + DEVICE_SHIFT);
#endif
		CUDA_CHECK_RETURN (cudaMemcpy (dbm[i].id_offsets, mst->id_offsets, sizeof(goffset_t) * (total_num_partitions + 1), cudaMemcpyHostToDevice));
		CUDA_CHECK_RETURN (cudaMemcpy (dbm[i].jid_offset, mst->jid_offset, sizeof(goffset_t) * total_num_partitions, cudaMemcpyHostToDevice));
	}
}
...
static void init_device_gather (dbmeta_t * dbm, master_t * mst, vid_t max_subsize, vid_t max_jsize, vid_t max_lsize)
{
	...
	for (i = 0; i < num_of_devices; i++)
	{
		...
		CUDA_CHECK_RETURN (cudaMalloc (&dbm[i].djs.spids, sizeof(ull) * max_jsize));
		CUDA_CHECK_RETURN (cudaMalloc (&dbm[i].djs.spidsr, sizeof(ull) * max_jsize));
		...
		CUDA_CHECK_RETURN (cudaMalloc (&dbm[i].dls.spids, sizeof(uint) * max_lsize));
...
//		cudaFree (dbm[i].nds.spids);
//		cudaFree (dbm[i].nds.spidsr);
		cudaFree (dbm[i].djs.edges);
		cudaFree (dbm[i].djs.kmers);
		cudaFree (dbm[i].djs.spids);
		cudaFree (dbm[i].djs.spidsr);
		int j;
		for (j=0; j<EDGE_DIC_SIZE; j++)
		{
			cudaFree (dbm[i].nb[j]);
...
static void init_device_graph_compute (meta_t * dm, master_t * mst)
{
	...
	for (i = 0; i < num_of_devices; i++)
	{
		...
		CUDA_CHECK_RETURN (cudaMalloc (&dm[i].edge.spid_nb, sizeof(uint) * gsize));
		CUDA_CHECK_RETURN (cudaMalloc (&dm[i].edge.spid_js, sizeof(uint) * gsize));
		...
		CUDA_CHECK_RETURN (cudaMemset (dm[i].edge.spid_nb, 0, sizeof(uint) * gsize));
		CUDA_CHECK_RETURN (cudaMemset (dm[i].edge.spid_js, 0, sizeof(uint) * gsize));
		...
		CUDA_CHECK_RETURN (cudaMalloc (&dm[i].id_offsets, sizeof(goffset_t) * (total_num_partitions + 1)));
		CUDA_CHECK_RETURN (cudaMalloc (&dm[i].jid_offset, sizeof(goffset_t) * total_num_partitions));

		CUDA_CHECK_RETURN (cudaMemcpy (dm[i].comm.id2index, mst->id2index[i], sizeof(int) * total_num_partitions, cudaMemcpyHostToDevice));
		CUDA_CHECK_RETURN (cudaMemcpy (dm[i].id_offsets, mst->id_offsets, sizeof(goffset_t) * (total_num_partitions + 1), cudaMemcpyHostToDevice));
		CUDA_CHECK_RETURN (cudaMemcpy (dm[i].jid_offset, mst->jid_offset, sizeof(goffset_t) * total_num_partitions, cudaMemcpyHostToDevice));
...
static void finalize_device_graph_compute (meta_t * dm, master_t * mst)
{
	...
	for (i = 0; i < num_of_devices; i++)
	{
		...
		cudaFree (dm[i].edge.spid_nb);
		cudaFree (dm[i].edge.spid_js);
...
static void realloc_device_junctions (meta_t * dm, master_t * mst)
{
	...
	for (i = 0; i < num_of_devices; i++)
	{
		...
		CUDA_CHECK_RETURN (cudaMalloc (&(dm[i].junct.spids), sizeof(uint) * spids_size));
		...
		CUDA_CHECK_RETURN (cudaMemset (dm[i].junct.spids, 0, sizeof(uint) * spids_size));
