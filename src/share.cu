...
void output_vertices_gpu (dbmeta_t * dbm, master_t * mst, voff_t jsize, voff_t lsize, int pid, int total_num_partitions, int did, d_jvs_t * djs, d_lvs_t * dls, subgraph_t * subgraph)
{
	...
	CUDA_CHECK_RETURN (cudaMemcpy (djs[partition_id].spids, dbm->djs.spids, sizeof(ull) * jsize, cudaMemcpyDeviceToHost));
	CUDA_CHECK_RETURN (cudaMemcpy (djs[partition_id].spidsr, dbm->djs.spidsr, sizeof(ull) * jsize, cudaMemcpyDeviceToHost));
	...
	CUDA_CHECK_RETURN(cudaMemcpy (dls[partition_id].spids, dbm->dls.spids, sizeof(uint) * lsize, cudaMemcpyDeviceToHost));
...
//					printf("Error in unitigs!\n");
				...
				{
					count_empty++;
//					printf ("GPU: from partition %d: junction id: %u (%u), neighbor th: %d, junction size=%u\n", pid, j+mst->id_offsets[pid], j, num, num_js);
//					continue;
				}
