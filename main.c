...
void preprocessing (char * contig_dir, subgraph_t * subgraph, dbtable_t * tbs, int total_num_partitions, int k, int p, int num_of_devices, int num_of_cpus, int world_size, int world_rank)
{
		...
		mst.id_offsets = (vid_t *)malloc(sizeof(vid_t) * (total_num_partitions+1));
		mst.jid_offset = (vid_t *)malloc(sizeof(vid_t) * total_num_partitions);
		memset (mst.id_offsets, 0, sizeof(vid_t) * (total_num_partitions + 1));
		memset (mst.jid_offset, 0, sizeof(vid_t) * total_num_partitions);
