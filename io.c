...
void allgather_subgraph_sizes (int total_num_partitions, d_jvs_t * js, d_lvs_t *ls, master_t * mst, int world_size, int world_rank)
{
	...
	int start_partition_id = np_per_node*world_rank;
	voff_t * tmp_id_offsets = (int *) malloc (sizeof(int) * num_of_partitions);
	voff_t * tmp_jid_offset = (int *) malloc (sizeof(int) * num_of_partitions);
	...
	mpi_allgatherv(tmp_id_offsets, mst->id_offsets + 1, total_num_partitions, world_size, world_rank);
	mpi_allgatherv(tmp_jid_offset, mst->jid_offset, total_num_partitions, world_size, world_rank);
	mst->id_offsets[0] = 0;
	if (world_size == 1)
	{
...
void get_junction_info_processors (master_t * mst, subgraph_t * subgraph)
{
	...
	int start_partition_id = mst->world_rank * np_per_node;
	int i;
