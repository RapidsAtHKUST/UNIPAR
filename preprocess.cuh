...
#endif
//		CUDA_CHECK_RETURN (cudaMemcpyToSymbol (sorted_kmers, &dbm[i].sorted_kmers, sizeof(ull*)));
...
//		CUDA_CHECK_RETURN (cudaMemcpyToSymbol (before_sort, &dbm[i].before_sort, sizeof(ull*)));
		...
		CUDA_CHECK_RETURN (cudaMemcpyToSymbol (id_offsets, &dbm[i].id_offsets, sizeof(vid_t *)));
		CUDA_CHECK_RETURN (cudaMemcpyToSymbol (jid_offset, &dbm[i].jid_offset, sizeof(vid_t *)));
...
/*
__global__ static void label_vertex_with_flags_binary2 (uint size, voff_t index_offset, int cutoff)
{
	const uint gid = blockIdx.x * blockDim.x + threadIdx.x;
	uint r, w;
	w = (size + TOTAL_THREADS_NODES - 1) / TOTAL_THREADS_NODES;
